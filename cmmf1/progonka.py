...
y_fake = fake()
y_solve = solve()
x, y_a = analytic_f(l, h)
print(Evaluate.Evaluate.get_error(y_a[0:-1], y_fake))
plt.plot(x[0:-1], y_solve, color='red', linewidth=1.0, linestyle='-', label=function_title)
plt.plot(x[0:-1], y_fake, color='blue', linewidth=1.0, linestyle='-', label='fake')
plt.plot(x[0:-1], y_a[0:-1], color='green', linewidth=1.0, linestyle='-', label='analytic')
